...
const ProjectsDisplay: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      const foundExperience = projectData.find((project) => project.id === Number(id));
      if (foundExperience) {
        setProjectState((prev) => ({ ...prev, selectedProject: foundExperience }));
      }
    } else if (formattedProjectList[0]) {
      setProjectState((prev) => ({ ...prev, selectedProject: formattedProjectList[0] }));
    }
  }, [id, formattedProjectList]);
    ...
    <div className='content__screen'>
      ...
      <div className='projects-display__container'>
        <div className='projects__list' ref={visibilityRef}>
          {formattedProjectList.map((project, i) => (
            <div
              key={project.id}
              onClick={() =>
                setProjectState({
